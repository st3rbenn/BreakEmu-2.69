def clean_enum_keys_and_convert_to_ts(input_enum_str, output_file_path):
    # Split the string into lines
    lines = input_enum_str.strip().split("\n")

    # Process each line to remove digits from the keys
    cleaned_lines = []
    for line in lines:
        if "=" in line:  # Process only lines with enum assignments
            key, value = line.split("=")
            cleaned_key = "".join([char for char in key if not char.isdigit()]).strip()
            cleaned_lines.append(f"    {cleaned_key} = {value.strip()},")
        else:
            cleaned_lines.append(line)  # Keep lines without '=' unchanged

    # Join the cleaned lines to form the new TypeScript enum
    cleaned_enum_str = "\n".join(cleaned_lines)

    # Write the cleaned TypeScript enum to the specified output file
    with open(output_file_path, "w") as file:
        file.write(cleaned_enum_str)


# Example TypeScript enum string
ts_enum_str = """
...
# Specify the output file path for the cleaned TypeScript enum
output_file_path = "cleanedInteractiveTypeEnum.ts"

# Call the function to process the enum and save the result
clean_enum_keys_and_convert_to_ts(ts_enum_str, output_file_path)

print(f"Cleaned enum has been saved to {output_file_path}")
