...
export default class BigEndianWriter extends ExtendedBuffer implements BinaryWriter {
    ...
    public writeVarLong(data: number): void {
        ...
        for (let i = 0; i < 4; i++) {
            this.writeByte(Number((low & BigInt(BigEndianWriter.MASK_01111111)) | BigInt(BigEndianWriter.MASK_10000000)));
        ...
        if ((high & 0xfffffff8n) === 0n) {
            ...
            this.writeByte(Number((((high << 4n) | low) & BigInt(BigEndianWriter.MASK_01111111)) | BigInt(BigEndianWriter.MASK_10000000)));

            high >>= 3n;

            while (high >= 0x80n) {
                this.writeByte(Number((high & BigInt(BigEndianWriter.MASK_01111111)) | BigInt(BigEndianWriter.MASK_10000000)));
