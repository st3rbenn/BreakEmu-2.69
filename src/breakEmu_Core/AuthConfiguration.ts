...
export class AuthConfiguration extends ServerConfiguration {
	...
	public async Load(): Promise<void> {
		return await new Promise<void>(async (resolve, reject) => {
			...
			if (fileExists) {
				...
				function calculateTotalParameters(config: IAuthConfiguration): number {
					let count = 0
				...
				for (const categorie in configResult) {
					...
					for (const value in ctg) {
						...
						let propertyName =
							categorie === "database" ||
							categorie === "authServer" ||
							categorie === "worldServer"
	...
	public async Save(): Promise<void> {
		const data: IAuthConfiguration = {
			...
			authServer: {
				...
				transitionUri: this.authServerTransitionUri,
			},
			worldServer: {
	...
	public Default(): void {
		...
		this.authServerTransitionUri = "amqp://localhost"
