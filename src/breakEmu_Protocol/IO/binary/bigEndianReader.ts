...
export default class BigEndianReader extends ExtendedBuffer implements BinaryReader {
    ...
    readVarInt(): number {
        ...
        while (size < BigEndianReader.INT_SIZE) {
            ...
            const bit: boolean = (byte & BigEndianReader.MASK_10000000) == BigEndianReader.MASK_10000000;
    ...
    readVarLong(): number {
        ...
        if ((lastByte & BigEndianReader.MASK_10000000) == BigEndianReader.MASK_10000000) {
            low |= (BigInt(lastByte) & BigInt(BigEndianReader.MASK_01111111)) << BigInt(size);
            ...
            while (size < 32) {
                ...
                if ((lastByte & BigEndianReader.MASK_10000000) === BigEndianReader.MASK_10000000) {
                    high |= (BigInt(lastByte) & BigInt(BigEndianReader.MASK_01111111)) << BigInt(size);
                } else {
                    break;
                }
