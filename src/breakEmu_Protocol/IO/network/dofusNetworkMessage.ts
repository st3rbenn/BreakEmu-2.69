...
export default class DofusNetworkMessage {
	...
	public static readHeader(
		reader: BinaryBigEndianReader
	): NetworkMessageHeader {
		const header: number = reader.readUShort()

		const messageId: number =
			header >> DofusNetworkMessage.BIT_RIGHT_SHIFT_LEN_PACKET_ID
	...
	public static writeHeader(
		writer: BinaryBigEndianWriter,
		messageId: number,
		messageWriter: BinaryBigEndianWriter
	): void {
		const type: number = DofusNetworkMessage.computeTypeLength(
			messageWriter.getPointer()
		)

		writer.writeUShort(
			(messageId << DofusNetworkMessage.BIT_RIGHT_SHIFT_LEN_PACKET_ID) | type
		)
